...
class BinanceClient:
    ...
    def get_pay_transactions(self, start_time_ms: int) -> list[dict]:
        """Binance Pay / internal-transfer history (both directions, wrapper response)."""
        payload = self._signed_request(
            "GET",
        ...
        if not isinstance(payload, dict):
            raise BinanceAPIError("Binance pay history response is not an object")
        code = str(payload.get("code", ""))
        if code != "000000" or not payload.get("success", False):
            raise BinanceAPIError(
                f"Binance pay history rejected: code={code}, message={payload.get('message')}",
                code=code,
            )
        ...
        if not isinstance(data, list):
            raise BinanceAPIError("Binance pay history data is not a list")
        return data
