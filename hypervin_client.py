...
class HypervinApi:
        ...
        try:
            ...
            for product in data["products"]:
                if not isinstance(product, dict):
                    return None
                product_id, name, price, stock = product.get("id"), product.get("name"), product.get("price"), product.get("stock")
                if product_id is None or not isinstance(name, str) or not isinstance(price, (int, float)) or not isinstance(stock, int):
                    ...
                    return None
                products[f"hv_{product_id}"] = {"name": name.strip(), "price": price, "stock": stock, "api_source": "HYPERVIN"}
            return products
    ...
    def create_order(self, product_id: str, quantity: int) -> dict[str, Any]:
        try:
            data = self.session.post(f"{self.base_url}/api/orders", json={"product_id": product_id, "quantity": quantity}, timeout=30).json()
            ...
            return {"success": True, "items": [str(item) for item in items], "total_charged": int(total_charged), "api_order_code": str(order_code)}
        except requests.exceptions.ConnectionError:
