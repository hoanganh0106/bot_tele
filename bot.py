...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user.id):
        text += (
            "\n🔧 **Lệnh Admin:**\n"
            "  /setprice — Sửa giá bán sản phẩm\n"
            "  /setmarkup — Đặt % markup mặc định\n"
            "  /prices — Xem bảng giá hiện tại\n"
...
async def cmd_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, info in products.items():
        ...
        btn_text = f"{info['name']} | {format_money(sell_price)} | {status}"
        btn = InlineKeyboardButton(btn_text, callback_data=f"prod_{key}")

        if "gpt" in key.lower() or "admingpt" in key.lower():
            categories["🤖 ChatGPT"].append([btn])
        elif "cc" in key.lower() or "capcut" in key.lower():
            categories["🎬 CapCut"].append([btn])
        elif "grok" in key.lower():
            categories["🔮 Grok"].append([btn])
        else:
            categories["🎨 Khác"].append([btn])

    buttons = []
    for cat_name, cat_buttons in categories.items():
        if cat_buttons:
            buttons.append([InlineKeyboardButton(f"━━━ {cat_name} ━━━", callback_data="noop")])
            buttons.extend(cat_buttons)
...
async def handle_product_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    qty_buttons.append([InlineKeyboardButton("⬅️ Quay lại", callback_data="back_menu")])
...
async def cmd_prices(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Xem bảng giá hiện tại (giá gốc vs giá bán)."""
    ...
    products, balance = api.get_stock()
    if not products:
        await update.message.reply_text("❌ Không lấy được dữ liệu từ API.")
        return

    default_markup = db.get_setting("default_markup_percent", 30)
    text = (
        f"💰 **BẢNG GIÁ HIỆN TẠI**\n"
        f"Markup mặc định: **{default_markup}%**\n"
        f"Số dư CTV: **{format_money(balance)}**\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
    )

    for key, info in products.items():
        base_price = info["price"]
        sell_price = get_sell_price(key, base_price)
        custom = db.get_custom_price(key)
        markup_info = f"(custom: {format_money(custom)})" if custom else f"(+{default_markup}%)"
        profit = sell_price - base_price

        text += (
            f"📦 **{info['name']}**\n"
            f"   Key: `{key}`\n"
            f"   Gốc: {format_money(base_price)} → Bán: {format_money(sell_price)} {markup_info}\n"
            f"   Lãi: {format_money(profit)}/cái | Kho: {info['stock']}\n\n"
        )
...
async def cmd_setprice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin sửa giá bán sản phẩm.
    Usage: /setprice <product_key> <giá_mới>
    Hoặc: /setprice <product_key> reset  (reset về markup mặc định)
    """
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Bạn không có quyền sử dụng lệnh này.")
        return
    ...
    if not args or len(args) < 2:
        ...
        text = (
            "📝 **SỬA GIÁ BÁN**\n\n"
            "Cú pháp: `/setprice <product_key> <giá_mới>`\n"
            "Reset: `/setprice <product_key> reset`\n\n"
            "**Danh sách product_key:**\n"
        )
        for key, info in products.items():
            sell_price = get_sell_price(key, info["price"])
            text += f"• `{key}` — {info['name']} | Bán: {format_money(sell_price)}\n"
    ...
    if price_str.lower() == "reset":
        db.remove_custom_price(product_key)
        await update.message.reply_text(
            f"✅ Đã reset giá `{product_key}` về markup mặc định.",
            parse_mode="Markdown"
        )
        return

    try:
        new_price = int(price_str.replace(",", "").replace(".", ""))
    except ValueError:
        await update.message.reply_text("❌ Giá không hợp lệ. Nhập số nguyên (VND).")
        return

    # Kiểm tra giá gốc
    products, _ = api.get_stock()
    if products and product_key in products:
        base = products[product_key]["price"]
        if new_price <= base:
            await update.message.reply_text(
                f"⚠️ Giá bán ({format_money(new_price)}) thấp hơn hoặc bằng giá gốc ({format_money(base)})!\n"
                f"Bạn sẽ bị lỗ. Vẫn muốn đặt? Gõ lại lệnh với giá cao hơn.",
                parse_mode="Markdown"
            )
            return
    ...
    await update.message.reply_text(
        f"✅ Đã cập nhật giá bán:\n"
        f"📦 `{product_key}`\n"
        f"💰 Giá mới: **{format_money(new_price)}**",
        parse_mode="Markdown"
    )
...
async def cmd_setmarkup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        ...
        await update.message.reply_text(
            f"📊 Markup mặc định hiện tại: **{current}%**\n\n"
            f"Cú pháp: `/setmarkup <phần_trăm>`\n"
            f"Ví dụ: `/setmarkup 50` = giá bán = giá gốc + 50%",
            parse_mode="Markdown"
        )
        return
    ...
    try:
        ...
        await update.message.reply_text("❌ Nhập số nguyên 0-500.")
        return

    db.set_setting("default_markup_percent", percent)
    await update.message.reply_text(
        f"✅ Markup mặc định đã đặt: **{percent}%**\n"
        f"(Chỉ áp dụng cho sản phẩm chưa set giá riêng)",
        parse_mode="Markdown"
    )
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"💵 Tổng doanh thu: **{format_money(stats['total_revenue'])}**\n"
        f"💸 Tổng giá gốc: **{format_money(stats['total_cost'])}**\n"
...
async def cmd_pending_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pending:
        await update.message.reply_text("✅ Không có đơn hàng nào đang chờ.")
        return
    ...
    for code, order in pending.items():
        text += (
            f"📋 `{code}`\n"
            f"   👤 {order.get('username', '?')} (ID: {order['user_id']})\n"
            f"   📦 {order.get('product_name', '?')} x{order['qty']}\n"
            f"   💰 {format_money(order['total'])}\n"
            f"   ⏰ {order.get('created_at', '?')[:16]}\n\n"
        )
        buttons.append([
            InlineKeyboardButton(f"✅ Xác nhận {code[-8:]}", callback_data=f"adminpay_{code}"),
...
# ============================================
# XỬ LÝ THANH TOÁN (gọi từ SePay webhook)
# ============================================
...
async def handle_email_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Nhận email từ khách cho slot_gpt_team."""
    user_id = update.effective_user.id
    ...
    if not waiting_order:
        return  # Không có đơn chờ email, bỏ qua
    ...
    order["emails"] = emails
    order["status"] = "pending"  # Reset để process_paid_order xử lý
    db.save_order(order_code, order)
...
def main():
    ...
    app.add_handler(CommandHandler("pendingorders", cmd_pending_orders))
    ...
    # Email input handler (for slot_gpt_team)
